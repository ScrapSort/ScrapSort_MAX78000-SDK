...
#define NUM_CLASSES 7 // number of output classes
#define NUM_OUTPUTS 7 // number of output neurons
...
output_classes_t class_names[] = {CAN, HEXAGON, TRAPEZOID, NONE, OTHER, BOTTLE, CUP};


char buff[TFT_BUFF_SIZE];
int font_1 = (int)&Arial12x12[0];
volatile uint32_t cnn_time; // Stopwatch
char* class_strings[] = {"CAN","HEXAGON","TRAPEZOID","NONE","OTHER","BOTTLE","CUP"};
...
cnn_output_t* run_cnn()
{
    ...
    for (int i = 0; i < NUM_CLASSES; i++) 
    {
      ...
      printf("[%7d] -> Class %s: %d.%d%%\n", ml_data[i], class_strings[i], digs, tens);
